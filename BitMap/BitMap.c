...
BOOL hyperrectangle_is_subset_of_bitmap(int_vec3_pair* r_min_max, const int_vec3_bitmap* bitmap) {
    int y = indexToInteger(&r_min_max->vect2, bitmap->Nx);
    if (!verify_vector_range(bitmap, indexToInteger(&r_min_max->vect2, bitmap->Nx))) {
        printf("outed v is (%d, %d, %d) which has %d\n", r_min_max->vect2.v1, r_min_max->vect2.v2, r_min_max->vect2.v3, y);
        return FALSE;
    }
...
int_vec3_arr bitmap_to_array(int_vec3_bitmap* bitmap) {
    int_vec3_arr vec3_arr;
    vec3_arr.data = (int_vec3*)malloc(sizeof(int_vec3) * bitmap->n_ons);
    vec3_arr.size = bitmap->n_ons;

    if (vec3_arr.data == NULL) {
        return NULL_INT_VEC3_ARR;
    }
    ...
    for (size_t p1 = 0; p1 <= bitmap->Nx->v1; p1++) {
        for (size_t p2 = 1; p2 <= bitmap->Nx->v2; p2++) {
            for (size_t p3 = 1; p3 <= bitmap->Nx->v3; p3++) {
                if (BITMAP_GET(bitmap->data, indexToInteger_integers(p1, p2, p3, bitmap->Nx))) {
                    if (p1 == 0) {
                        vec3_arr.data[vec_idx] = INT_VEC3_ZERO;
                    } else {
                        vec3_arr.data[vec_idx] = (int_vec3){p1, p2, p3};
                    }
                    vec_idx++;
                }
            }

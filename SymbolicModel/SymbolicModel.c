            ...
            for (int p3 = 1; p3 <= NX_3; p3++) {
                for (int c1 = 1; c1 <= NU_1; c1++) {
                    for (int c2 = 1; c2 <= NU_2; c2++) {
                        ...
                        u_sigma = p(&(int_vec2){c1, c2}, &Nu, &u_min, &u_max);

                        f_min_max = get_reachable_region(&p_x_min_max.vect1, &p_x_min_max.vect2, &u_sigma, &w_min, &w_max, reachability_method, Dx, Dw);

                        correct_f_angles(&f_min_max.vect1);
                        correct_f_angles(&f_min_max.vect2);

                        correct_f_min_max(&f_min_max);
...
void correct_f_min_max(float_vec3_pair* f_min_max) {
    ...
    if (f_min_max->vect1.v1 > f_min_max->vect2.v1) {
        temp = f_min_max->vect2.v1;
        f_min_max->vect2.v1 = f_min_max->vect1.v1;
        f_min_max->vect1.v1 = temp;
    }
    if (f_min_max->vect1.v2 > f_min_max->vect2.v2) {
        temp = f_min_max->vect2.v2;
        f_min_max->vect2.v2 = f_min_max->vect1.v2;
        f_min_max->vect1.v2 = temp;
    }
    if (f_min_max->vect1.v3 > f_min_max->vect2.v3) {
        temp = f_min_max->vect2.v3;
        f_min_max->vect2.v3 = f_min_max->vect1.v3;
        f_min_max->vect1.v3 = temp;
    }
}
    ...
    for (size_t p1 = 1; p1 <= Nx.v1; p1++) {
        for (size_t p2 = 1; p2 <= Nx.v2; p2++) {
            for (size_t p3 = 1; p3 <= Nx.v3; p3++) {
                ...
                if (exists_sigma_st_g_ksi_sigma_is_pre(m, ksi, R)) {
